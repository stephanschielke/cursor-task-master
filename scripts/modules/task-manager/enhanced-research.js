	...
	try {
		...
		if (useWebSearch) {
			if (outputFormat === 'text') {
				console.log(
					chalk.gray(
						'🔍 Query benefits from web search, fetching current information...'
					)
				);
			}
			...
			try {
				...
				if (outputFormat === 'text' && webSearchResults.results.length > 0) {
					console.log(
						chalk.green(
							`✅ Found ${webSearchResults.results.length} relevant web results`
						)
					);
				}
			} catch (webSearchError) {
				mcpLog?.warn(
					`Web search failed, continuing with local research: ${webSearchError.message}`
				);
				webSearchResults = {
					results: [],
					content: '',
...
async function createEnhancedResearchResult(
	...
	try {
		...
		const { systemPrompt, userPrompt } = await promptManager
			.loadPrompt(
 ...
 */
function displayEnhancedResearchResults(
	result,
	query,
	detailLevel,
	webSearchResults,
