...
async function searchWithMCP(query, options = {}) {
	...
	try {
		...
		const { mcp_web_search_sse_search } = await import(
			'../../mcp-client.js'
		).catch(() => {
...
async function fetchGithubReadme(url) {
	try {
		...
		const { mcp_web_search_sse_fetchGithubReadme } = await import(
			'../../mcp-client.js'
		).catch(() => {
 ...
 */
function integrateWebSearchContext(
	originalQuery,
	gatheredContext,
	webSearchResults,
	detailLevel
	...
	enhancedContext += '\n\n' + '-'.repeat(40) + '\n';
	enhancedContext +=
		'INSTRUCTION: When responding, integrate the above web search results\n';
	enhancedContext +=
		'with the project context. Prioritize current/recent information from\n';
	enhancedContext +=
		'web results while relating it back to the specific project needs.\n';
