...
describe('CursorAgentProvider AI SDK Methods', () => {
	...
	describe('generateText() - Usage Metrics & Response Handling', () => {
		...
		it('should propagate progress tracker updates', async () => {
			...
			expect(mockProgressTracker.updateProgress).toHaveBeenCalledWith(
				0,
				'Preparing cursor-agent request'
			);
			expect(mockProgressTracker.updateProgress).toHaveBeenCalledWith(
				0.1,
			...
			expect(mockProgressTracker.updateProgress).toHaveBeenCalledWith(
				0.9,
				'Processing cursor-agent response'
			);
			expect(mockProgressTracker.updateTokensWithCost).toHaveBeenCalledWith(
				60,
				40,
				0,
				0,
			...
			expect(mockProgressTracker.complete).toHaveBeenCalledWith(
	...
	describe('generateObject() - Schema Instructions & JSON Parsing', () => {
		...
		it('should build correct schema instructions for newTaskData', async () => {
			const schemaInstructions = provider._buildSchemaInstructions(
				{},
				'newTaskData'
			);

			expect(schemaInstructions).toContain(
	...
	describe('Model Mapping Validation', () => {
		...
		it('should return original model ID for unmapped models', () => {
			expect(provider.mapModelIdToCursorAgent('unknown-model')).toBe(
				'unknown-model'
			);
