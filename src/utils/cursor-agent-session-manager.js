...
export class CursorAgentSessionManager {
	...
	async cleanupSession(sessionId, force = false) {
		...
		try {
			...
			this.unregisterSession(
				sessionId,
				force ? 'forced_cleanup' : 'normal_cleanup'
			);
		...
		for (const [sessionId, session] of this.activeSessions) {
			...
			if (age > this.maxSessionAge || inactivity > session.timeoutMs + 30000) {
				sessionsToCleanup.push({
					sessionId,
					reason:
						age > this.maxSessionAge ? 'max_age_exceeded' : 'timeout_exceeded',
					age,
					inactivity
				});
			}
	...
	async cleanupOrphanedCursorAgentProcesses(projectRoot = null) {
		try {
			...
			const orphanedProcesses = processes.filter((proc) => {
				...
				const isTracked = Array.from(this.activeSessions.values()).some(
					(session) => session.pid == proc.pid
				);
	...
	getSessionStats() {
		...
		const stats = {
			...
			avgSessionAge:
				sessions.length > 0
					? sessions.reduce((sum, s) => sum + (currentTime - s.startTime), 0) /
						sessions.length
					: 0,
			operationTypes: {},
			oldestSession: null,
		...
		sessions.forEach((session) => {
			...
			if (
				!stats.oldestSession ||
				age > currentTime - stats.oldestSession.startTime
			) {
				stats.oldestSession = {
