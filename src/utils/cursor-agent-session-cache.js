...
export function getCachedChatId(projectRoot, model) {
	...
	if (!session || shouldInvalidateSession(session)) {
		if (session) {
			log(
				'info',
				`Session invalidated due to failures: ${session.resumeAttempts || 0} attempts`
			);
	...
	log(
		'info',
		`Using stored cursor-agent session: ${session.chatId} (age: ${ageInDays} days)`
	);
...
export function cacheChatId(projectRoot, model, chatId, isNew = true) {
	...
	log(
		'info',
		`Stored cursor-agent session: ${chatId} (total: ${Object.keys(cleanedSessions).length})`
	);
}
...
export function markResumeFailure(projectRoot, model, failedChatId) {
	...
	log(
		'warn',
		`Cursor-agent resume failure: ${failedChatId} (attempt ${session.resumeAttempts}/${config.maxResumeAttempts})`
	);
	...
	if (session.resumeAttempts >= config.maxResumeAttempts) {
		...
		log(
			'info',
			`Removed cursor-agent session after max failures: ${failedChatId}`
		);
	} else {
...
export function getCacheStats(projectRoot) {
	...
	if (oldestSession) {
		stats.oldestSessionDays = Math.round(
			(now - oldestSession) / (1000 * 60 * 60 * 24)
		);
		stats.newestSessionDays = Math.round(
			(now - newestSession) / (1000 * 60 * 60 * 24)
		);
	}
