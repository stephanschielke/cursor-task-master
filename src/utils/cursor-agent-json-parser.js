...
function parseCursorAgentOutput(output, isResearchOperation = false) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        try {
            ...
            if (trimmedLine.includes('result') || trimmedLine.includes('session_id')) {
                console.log('[PARSER-DEBUG] Failed to parse potentially important line:', {
                    lineNumber: i,
                    error: e.message,
    ...
    if (!resultObject) {
        ...
        if (resultTypeMatch) {
            console.log('[PARSER-DEBUG] Found "type":"result" at position', resultTypeMatch.index);
            ...
            if (braceCount === 0) {
                ...
                console.log('[PARSER-DEBUG] Extracted JSON string length:', jsonStr.length);
                ...
                try {
                    ...
                    console.warn('[PARSER-DEBUG] Strategy 2 JSON parse failed:', parseError.message);
                }
            } else {
                console.warn('[PARSER-DEBUG] Strategy 2: Unmatched braces, braceCount =', braceCount);
            }
        } else {
            console.log('[PARSER-DEBUG] Strategy 2: No "type":"result" pattern found');
        }
    }
    ...
    if (!resultObject) {
        ...
        console.warn('   - Outdated TaskMaster CLI version (update with: npm install -g task-master-ai@latest)');
    ...
    console.log('[PARSER-DEBUG] - Preview (first 300 chars):', actualResult.substring(0, 300));
    console.log('[PARSER-DEBUG] - Last 100 chars):', actualResult.substring(Math.max(0, actualResult.length - 100)));
    ...
    if (typeof actualResult === 'string' && actualResult.length > 10) {
        ...
        const looksLikeJson = (actualResult.startsWith('[') || actualResult.startsWith('{') ||
            actualResult.startsWith('"[') || actualResult.startsWith('"{'));
        ...
        if (looksLikeJson && (hasEscapedQuotes || hasEscapedNewlines)) {
            try {
                console.log('[PARSER-DEBUG] Attempting to parse double-encoded JSON result');
                ...
                console.log('[PARSER-DEBUG] Parsed result preview:', JSON.stringify(parsedResult).substring(0, 200));
                actualResult = parsedResult;
            } catch (parseError) {
                console.warn('[PARSER-DEBUG] Failed to parse double-encoded JSON:', parseError.message);
                ...
                if (actualResult.startsWith('"') && actualResult.endsWith('"')) {
                    ...
                    try {
                        ...
                        console.warn('[PARSER-DEBUG] Still failed after quote removal:', cleanError.message);
                    }
                }
            }
        }
